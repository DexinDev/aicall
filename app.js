...
app.post('/gather', async (req, res) => {
  ...
  try {
    ...
    if (st.stage === 'route') {
      ...
      if (intent === 'job') {
        st.stage = 'alt_job';
        const v = await tts(`Awesome — we’re always looking for good people. Please visit our website and go to the Careers section. You can apply there, and our hiring team will get in touch directly. Thanks for calling!`);
        calls.delete(sid);
    ...
    if (st.stage === 'addr') {
      ...
      if (!free.length) {
        const v = await tts(`Sorry, I don’t see any open time in the next days. Would you like me to leave a message for a call back?`);
        calls.delete(sid);
